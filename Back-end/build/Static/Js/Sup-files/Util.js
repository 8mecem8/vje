...
export const apiUrl = document.location.href.startsWith('http://localhost')? 'http://localhost:5000'
  : window.location.href.includes('') ? /* `${window.location.hostname}/buyozon`  */ " "
...
export const deleteOrder= async (productId) => {
  try {
    ...
    const response = await axios.delete(`${apiUrl}/api/order/${productId}`,config);
...
export const deliverOrder = async (orderId) => {
  try {
    ...
    const response = await axios.put(`${apiUrl}/api/order/${orderId}/deliver`,config);
...
export const deliverOrdero= async (productId) => {
  try {
    ...
    const response = await axios.put(`${apiUrl}/api/order/${productId}/deliver`,name,config);
...
export const getSummary = async () => {
  try {
    ...
    const response = await axios.get(`${apiUrl}/api/order/summary`,config);
